...
app.use(cors({
  origin: '*'  // O el origen del frontend si es diferente
}));
...
app.post('/api/login', (req, res) => {
  const { numControl, contrasena } = req.body;
  console.log('Cuerpo de la solicitud:', req.body);

  // Verificar si numControl o contrasena están vacíos
  if (!numControl || !contrasena) {
    return res.status(400).send({ message: 'Número de control y contraseña son requeridos' });
  }
  ...
  db.query(
    ...
    async (err, result) => {
      ...
      const usuario = result[0]; // Revisa que este valor sea el correcto
      console.log('Usuario encontrado:', usuario);  // Muestra el usuario encontrado en la base de datos
      ...
      // **Depuración: Imprime las contraseñas** para ver lo que se recibe y lo que se compara
      console.log('Contraseña recibida desde el frontend:', contrasena);
      console.log('Contraseña almacenada en la base de datos (encriptada):', usuario.contrasena);

      // Compara la contraseña ingresada con la almacenada
      try {
        // Realiza la comparación usando bcrypt
        const contrasenaMatch = await bcrypt.compare(contrasena, usuario.contrasena);

        console.log('¿Contraseña válida?', contrasenaMatch); // Verifica si la contraseña coincide
        ...
        if (contrasenaMatch) {
          ...
          return res.status(200).send({ token: randomToken.toString(), userData: usuario });
        } else {
...
app.post('/api/reset-password', (req, res) => {
  ...
  bcrypt.hash(newPassword, 10, (err, hash) => {
    ...
    const sqlQuery = "UPDATE usuario SET contrasena = ? WHERE correo = ?";
...
app.put('/api/actualizar/usuarios/:numControl', (req, res) => {
  ...
  db.query(sQuery, [nuevosDatos, libroId], (err, results) => {
    if (err) {
...
app.get('/api/actualizar/usuarios/:numControl', (req, res) => {
  ...
  db.query(sQuery, [numControl], (err, results) => {
      if (err) {
...
app.put('/api/actualizar/usuario/:numControl', (req, res) => {
  ...
  // Ejemplo:
  db.query('UPDATE usuario SET nombre = ?, apellido = ?, correo = ?, telefono = ? WHERE numControl = ?', 
...
app.delete('/api/eliminar/usuarios/:numControl', (req, res) => {
  ...
  db.query(query, [numControl], (err, results) => {
    if (err) {
...
app.get('/api/libros/:id', (req, res) => {
    ...
    db.query(sQuery, [isbn], (err, results) => {
        if (err) {
...
app.post('/api/libros', (req, res) => {
    ...
    db.query(sQuery, [libro], (err, results) => {
        if (err) {
...
app.get('/api/prestamos/:id', (req, res) => {
    ...
    db.query(sQuery, [idPrestamo], (err, results) => {
        if (err) {
...
app.post('/api/prestamos', (req, res) => {
  ...
  db.query(sQueryVerificar, [idEjemplar], (err, results) => {
      if (err) {
          ...
          if (count > 0) {
              ...
              db.query(sQuery, [prestamo], (err, results) => {
                  if (err) {
...
app.put('/api/prestamos/:id', (req, res) => {
    ...
    db.query(sQuery, [fechaDevolucion, idPrestamo], (err, results) => {
        if (err) {
            console.error('Error al eliminar el prestamo: ' + err.message);
            res.status(500).send('Error al eliminar el prestamo de la base de datos');
        } else {
...
app.get('/api/ejemplares/:id', (req, res) => {
    ...
    db.query(sQuery, [idEjemplar], (err, results) => {
        if (err) {
            ...
            if (results.length > 0) {
                ...
                db.query(sQueryLibro, [idEjemplar], (err, libroResults) => {
                    if (err) {
...
app.get('/api/libros/buscar', (req, res) => {
  ...
  db.query(sQuery, [searchTerm], (err, resultados) => {
    if (err) {
...
// Ruta para insertar ejemplar y actualizar libro
app.post('/api/ejemplares/', (req, res) => {
  ...
  db.getConnection((err, connection) => {
    ...
    connection.beginTransaction((err) => {
      ...
      connection.query(sQueryInsertEjemplar, ejemplar, (err, results) => {
        if (err) {
          // Si hay un error, hacemos rollback
          connection.rollback(() => {
            console.error('Error al agregar el ejemplar: ' + err.message);
            connection.release();  // Liberamos la conexión
            return res.status(500).send('Error al insertar el ejemplar en la base de datos');
          });
        } else {
          // Si el ejemplar se insertó correctamente, actualizamos el campo cantEjemplares en la tabla Libro
          const { ISBN } = ejemplar;
          connection.query(sQueryUpdateCantEjemplares, ISBN, (err, results) => {
            if (err) {
              // Si hay un error en la actualización, hacemos rollback
              connection.rollback(() => {
                console.error('Error al actualizar cantEjemplares: ' + err.message);
                connection.release();  // Liberamos la conexión
                return res.status(500).send('Error al actualizar cantEjemplares en la tabla Libro');
              });
            } else {
              ...
              connection.commit((err) => {
                if (err) {
                  ...
                  connection.rollback(() => {
                    ...
                    connection.release();  // Liberamos la conexión
                    return res.status(500).send('Error al realizar commit de la transacción');
                  });
                } else {
                  // Responder con el resultado exitoso
                  res.json(results);
                  connection.release();  // Liberamos la conexión después de la transacción
                }
              });
            }
          });
