...
export class LoginComponent implements OnInit {
  ...
  login(loginForm: NgForm) {
    ...
    console.log('numControl:', this.numControl);
    console.log('contrasena:', this.contrasena);

    if (!this.numControl || !this.contrasena) {
      Swal.fire('¡Número de control y contraseña son requeridos!', '', 'error')
      this.errorMensaje = 'Número de control y contraseña son requeridos';
      return;
    }
    ...
    this.usuariosService.login(this.numControl, this.contrasena).subscribe(
      (res: any) => {
        ...
        Swal.fire('¡Bienvenido!', 'Esperamos que leer sea tu mayor inspiraciòn', 'success')
      },
      (error: any) => {
  ...
  //   this.http.post('http://localhost:8080/api/verificar-token', { token: this.codigoVerificacion })
  //     .subscribe(
  //       (response: any) => {
